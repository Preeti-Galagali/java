...
/**
 * EmployeeService class provides business logic for employee management. It
 * includes operations like adding, removing, updating, searching, filtering,
 * and sorting employees.
 */
public class EmployeeService {
	...
	public void updateSalary(int empID, double empSalary) {
		employees.stream().filter(emp -> emp.getId() == empID).findFirst().ifPresentOrElse(emp -> {
	...
	public void searchByEmployeeID(int empID) {
		employees.stream().filter(emp -> emp.getId() == empID).findFirst().ifPresentOrElse(System.out::println,
				() -> System.out.println("No Employee Records Found."));
	}
}